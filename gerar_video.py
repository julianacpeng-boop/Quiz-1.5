...
from campanha import VIDEOS

# O gerador carrega estes valores para cada vídeo, um de cada vez.
TEMA = ""
HASHTAG = ""
GANCHO = ""
PERGUNTAS = []
...
# =========================================================
def validar_video(video, indice):
    obrigatorios = [
        "layout",
        "tema",
        "hashtag",
    ...
    for campo in obrigatorios:
        if campo not in video:
            raise ValueError(
                f"Vídeo {indice}: falta o campo '{campo}' em campanha.py."
            )
    ...
    if layout not in THEMES:
        raise ValueError(
            f"Vídeo {indice}: layout deve ser um número de 1 a 6."
        )

    perguntas = video["perguntas"]

    if len(perguntas) != 5:
        raise ValueError(
            f"Vídeo {indice}: use exatamente 5 perguntas."
        )

    for i, q in enumerate(perguntas, start=1):
        if len(q.get("opcoes", [])) != 4:
            raise ValueError(
                f"Vídeo {indice}, pergunta {i}: use exatamente 4 opções."
            )

        if q.get("correta") not in (0, 1, 2, 3):
            raise ValueError(
                f"Vídeo {indice}, pergunta {i}: "
                "'correta' deve ser 0, 1, 2 ou 3."
            )

        if not q.get("pergunta"):
            raise ValueError(
                f"Vídeo {indice}, pergunta {i}: texto vazio."
            )

        if not q.get("explicacao"):
            raise ValueError(
                f"Vídeo {indice}, pergunta {i}: explicação vazia."
            )


def carregar_video(video):
    global TEMA, HASHTAG, GANCHO, PERGUNTAS, FECHAMENTO

    TEMA = video["tema"]
    HASHTAG = video["hashtag"]
    GANCHO = video["gancho"]
    PERGUNTAS = video["perguntas"]
    FECHAMENTO = video["fechamento"]


def nome_seguro(texto):
    texto = re.sub(r"[^A-Za-z0-9_-]+", "_", str(texto))
    return texto.strip("_") or "video"


# =========================================================
# GERAÇÃO DE UM VÍDEO
# =========================================================
async def gerar_um_video(video, numero):
    validar_video(video, numero)
    carregar_video(video)

    layout = int(video["layout"])
    theme = THEMES[layout]

    video_id = nome_seguro(
        video.get("id", f"video_{numero:02d}")
    )

    temp = TEMP_ROOT / f"{numero:02d}_{video_id}_layout_{layout}"
    ...
    print()
    print("========================================")
    print(f"INICIANDO VÍDEO {numero}")
    print("ID:", video_id)
    print("Tema:", TEMA)
    print("Hashtag:", HASHTAG)
    print("Layout indicado:", layout, "-", theme["nome"])
    print("========================================")
    ...
    for qi, q in enumerate(PERGUNTAS):
        ...
        for numero_contagem in range(
            ...
            contagem_img = (
                temp
                / f"q{qi+1}_contagem_{numero_contagem}.png"
            )
            ...
            segmentos.append(
                segmento_silencio(
                    contagem_img,
                    temp,
                    f"q{qi+1}_bip_{numero_contagem}",
                    1.0,
                    beep=True,
                )
            )
        ...
        # EXPLICAÇÃO VISUAL, SEM NARRAÇÃO
    ...
    tema_nome = nome_seguro(TEMA)

    saida = (
        OUTPUT
        / (
            f"{numero:02d}_{video_id}_"
            f"{tema_nome}_LAYOUT_{layout}.mp4"
        )
    )
...
# =========================================================
# GERAÇÃO DE TODOS OS VÍDEOS CADASTRADOS
# =========================================================
async def main():
    if not isinstance(VIDEOS, (list, tuple)):
        raise ValueError(
            "Em campanha.py, VIDEOS deve ser uma lista."
        )

    if not VIDEOS:
        raise ValueError(
            "Nenhum vídeo foi cadastrado em campanha.py."
        )

    # Evita misturar arquivos antigos com a execução atual.
    if OUTPUT.exists():
