...
# ============================================================
# JUH QUIZ NEON — baseado no Quiz-vers-o-1.3
#
# MODO_LAYOUT:
...
try:
    # Reaproveita exatamente os 45 temas/perguntas do gerador 1.3.
    # Assim você não precisa duplicar as 225 perguntas neste arquivo.
    from gerar_videos_game_show import QUIZZES
except Exception as e:
    raise RuntimeError(
        "Não consegui importar QUIZZES de gerar_videos_game_show.py. "
        "Deixe os dois arquivos na raiz do mesmo repositório."
    ) from e
...
# Quantidade opcional.
QUANTIDADE_VIDEOS = int(os.getenv("QUANTIDADE_VIDEOS", str(len(QUIZZES))))
QUANTIDADE_VIDEOS = max(1, min(QUANTIDADE_VIDEOS, len(QUIZZES)))
...
def render_frame(tema, pergunta, numero, estado, timer=None):
    ...
    for i in range(5):
        ...
        if i < numero - 1:
            ...
            cor = (196, 209, 236)
        draw.ellipse([cx-8, dot_y-8, cx+8, dot_y+8], fill=cor)
    ...
    for tema in temas[:QUANTIDADE_VIDEOS]:
        ...
        for q in perguntas[:PERGUNTAS_POR_VIDEO]:
            if "pergunta" not in q:
                raise ValueError(f"Tema '{tema}' tem item sem 'pergunta'.")

            if len(q.get("alternativas", [])) != 3:
                raise ValueError(
                    f"Pergunta '{q.get('pergunta', '')}' precisa ter 3 alternativas."
                )

            if int(q.get("correta", -1)) not in (0, 1, 2):
                raise ValueError(
                    f"Pergunta '{q.get('pergunta', '')}' tem resposta inválida."
                )
...
def main():
    ...
    print(f"Vídeos: {QUANTIDADE_VIDEOS}", flush=True)
    print("=" * 72, flush=True)
    ...
    temas = list(QUIZZES.items())[:QUANTIDADE_VIDEOS]

    for video_num, (tema, perguntas) in enumerate(temas, start=1):
        perguntas_tema = perguntas[:PERGUNTAS_POR_VIDEO]
        ...
        render_intro(tema).save(frame_tema)

        audio_tema = pasta_video / "00_tema.mp3"
        fala_tema = f"Tema do quiz: {tema}. Cinco perguntas. Vamos começar."
